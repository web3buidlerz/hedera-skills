...
## Seed / project layout and secrets (judgment)
...
- [ ] **O12** **legacy clone:** `seed.repo` / `seed.ref` sensible; `isolation.neverModifySeedRepo: true`
      when using a shared seed. **project:** no `seed` block; workspace is the project cwd
- [ ] **O13** Command timeouts are generous enough for cold CI (install ≥ 300s typical);
      `requiresNoSecrets: true` (or equivalent)
- [ ] **O14** **project:** `extend.baseline` lists host-health commands with a literal
      `"install"` name; baseline is separate from post-feature `validators.commands`
- [ ] **O15** **project:** `requiredFiles` include `.harness/spec.yaml` / `.harness/prd.md`
      (and contract when gate 3); `templateMetadata.name` may differ from the feature
      slug (host template identity)

## Quick oracle-blocker shortlist

Fail the review as not-runnable if **O1**, **O2**, or **O10** fail (plus any
Wiring blockers from `check-spec.sh`). For **project**, also treat missing
